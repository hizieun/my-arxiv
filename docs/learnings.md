...
**원인:** arXiv export API가 우리 IP에 **429 "Rate exceeded"**(IP 기반 rate limit). 카테고리 수와 무관 — `cat:cs.AI` 1건 쿼리도 429. 개발 중 반복 호출(preview 테스트, curl 진단)로 버스트 한도 초과 → 일시 차단. 게다가 차단 중 arXiv가 429를 **가끔 매우 느리게(15s+)** 응답해, 단순 재시도가 지연을 29초까지 누적시킴.
- arXiv 권장: 요청 간 3초 간격, 식별 가능한 User-Agent.

**해결:**
- `lib/arxiv.ts` `arxivFetch()` — 429/5xx에 한해 지수 백오프(500→1000ms) **재시도 3회**. 순간 버스트 429를 대부분 흡수.
- 각 시도에 **AbortController 타임아웃 6s** — 느린 429 응답을 빨리 끊어 다음 시도로. (정상 응답은 1s 미만이라 무영향) 라우트 29s → 9.3s.
- User-Agent를 `my-arxiv/0.4 (+repo URL)`로 통일(식별 가능).
- **UX**: `StatusStrip`에서 "⚠ arXiv/HF 실패" 칩 제거. 한쪽 소스 실패는 HF·캐시로 폴백하며 조용히 처리. 둘 다 실패해 결과 0일 때만 본문 안내(bothError).

**예방/탐지:** arXiv는 버스트에 민감 → 개발 중 반복 호출 자제, 진단 curl도 간격 두기. 프로덕션(Vercel)은 ISR 캐시(`revalidate: 1800`)로 호출 빈도가 낮아 429가 드묾. 외부 소스 실패는 "경고"가 아니라 "폴백+무알림"이 기본.
